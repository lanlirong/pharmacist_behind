...
use app\index\model\consult\Question as ConsultM;
use app\index\model\consult\Raw_question as Raw_ConsultM;
...
class Check extends Controller
{
    public function getRawList()
    {
        ...
        $list = $Raw_ConsultM->where($sql2)->where($sql)->where($sql3)->order($order)->paginate($params->size, false, [
            'page' =>  $params->page,
        ]);
    ...
    public function getCheckByID()
    {
        ...
        if ($params == [] || $params['id'] == '') {
            ...
            $Raw_ConsultM = new Raw_ConsultM();
            $disease = $Raw_ConsultM->where('id', $params['id'])->find();
            if ($disease) {
                $result = array(
                    'data' => $disease,
                    'code' => 1,
    ...
    public function check()
    {
        ...
        if ($params['status'] == 1) {
            $Raw_ConsultM = new Raw_ConsultM();
            $raw = $Raw_ConsultM->where('id', $params['id'])->find();
            if ($raw &&  $raw->isNew) { //更新
                $raw->id = $raw->isNew;
                $disease = json_decode(json_encode($raw), true);
                $ConsultM = new ConsultM();
                $ConsultM->allowField(true)->save($disease, ['id' => $raw->id]);
            } else {  // 新增
                $raw->id = Get8Str(6);
                // unset($raw->id);
                $raw->reviewer = $_SESSION['userInfo']['username'];
                $disease = json_decode(json_encode($raw), true);
                $ConsultM = new ConsultM($disease);
                $ConsultM->allowField(true)->save();
            }
